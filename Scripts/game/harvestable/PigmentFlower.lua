...
function PigmentFlower.server_onMelee(self, hitPos, attacker, damage)
	if not self.harvested and sm.exists(self.harvestable) then
		...
		if SurvivalGame then
			...
			sm.projectile.harvestableCustomProjectileAttack(harvest, "loot", 0, pos,
				sm.noise.gunSpread(sm.vec3.new(0, 0, 1), 20) * 5, self.harvestable, 0)
		end
		sm.harvestable.create(hvs_farmables_growing_pigmentflower, self.harvestable.worldPosition,
			self.harvestable.worldRotation)
	...
	if not self.harvested and sm.exists(self.harvestable) then
		if SurvivalGame then
			...
			if sm.container.beginTransaction() then
				sm.container.collect(container, obj_resource_flower, 5)
				if sm.container.endTransaction() then
					sm.event.sendToPlayer(player, "sv_e_onLoot",
						{ uuid = obj_resource_flower, pos = self.harvestable.worldPosition })
					sm.effect.playEffect("Pigmentflower - Picked", self.harvestable.worldPosition)
					sm.harvestable.create(hvs_farmables_growing_pigmentflower, self.harvestable.worldPosition,
						self.harvestable.worldRotation)
					sm.harvestable.destroy(self.harvestable)
