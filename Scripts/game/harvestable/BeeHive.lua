...
function BeeHive.server_onProjectile(self, hitPos, hitTime, hitVelocity, _, attacker, damage, userData, hitNormal,
																		 projectileUuid)
	self:sv_onHit()
end
...
function BeeHive.sv_onHit(self)
	if not self.destroyed and sm.exists(self.harvestable) then
		...
		sm.harvestable.createHarvestable(hvs_farmables_beehive_broken, self.harvestable.worldPosition,
