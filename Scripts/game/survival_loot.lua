...
local random_loot = {
	...
	{ uuid = obj_resource_circuitboard, chance = 50, quantity = 10 },
...
local loot_crate_startarea = {
	...
	selectOne = {
		...
		{ uuid = obj_resource_circuitboard, chance = 1, quantity = 10 },
...
local loot_haybot = {
	...
	randomLoot = {
		{ uuid = obj_consumable_component,  chance = 1 },
		{ uuid = obj_resource_circuitboard, chance = 2, quantity = 10 },
	}
}
...
function SpawnLoot(origin, lootList, worldPosition, ringAngle)
	...
	if worldPosition then
		for i = 1, #lootList do
			...
			elseif type(origin) == "Player" or type(origin) == "Unit" then
				sm.projectile.customProjectileAttack(params, projectileName, 0, worldPosition, vel, origin, worldPosition,
					worldPosition, 0)
			elseif type(origin) == "Harvestable" then
