...
function SurvivalGame.bindChatCommands( self )
	...
	sm.game.bindChatCommand("/lgdevmode", {{"bool", "enable", false}}, "cl_onLgChatCommand", "Toggles dev mode")
...
function SurvivalGame.loadCraftingRecipes( self )
	LoadCraftingRecipes({
		...
		craftbot = "$CONTENT_DATA/CraftingRecipes/craftbot.jsonc",
...
function SurvivalGame.cl_onLgChatCommand( self, params)
	if params[1] == "/lgdevmode" then
		local state = params[2]
		sm.log.info("Before: ".. tostring(self.sv.saved.data.dev))
		self.sv.saved.data.dev=state
		sm.log.info("After: ".. tostring(self.sv.saved.data.dev))
		sm.gui.chatMessage("Set devmode state to " .. tostring(state) .. ". It will take effect the next time you enter the game")
